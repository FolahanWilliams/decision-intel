...
SELLER-DECK / BROKER-NARRATIVE TELLS (sharpening of the ADVERSE-SELECTION filter above) — the thesis is almost always built on a broker's CIM or a seller-prepared deck, so screen the SELLER's narrative for the specific rhetorical devices that mask risk. Flag each as a risk indicator to PRESSURE-TEST in diligence before the searcher spends broken-deal cost — name a missing verification, never assert the seller is lying (the Seller-Halo Filter names an information asymmetry, not deceit):
- SMOOTHED CYCLICALITY — a trailing revenue/EBITDA line presented as steady growth when the underlying business is seasonal or cyclical. Flag a multi-year CAGR or "consistent growth" claim with no segment- or period-level volatility disclosed (construction, hospitality, ag, discretionary-consumer especially). (Survivorship + Anchoring.)
- DIVERSIFICATION-THAT-ISN'T — revenue called "diversified" while the named customers, end-markets, or case studies cluster in one cyclical industry or one channel. Flag a diversification claim whose own supporting evidence is concentrated. (Illusion of Validity + Halo Effect.)
- GROWTH-OPPORTUNITY-AS-OWED — "untapped white space," "just needs a motivated owner" framed as value priced into today's multiple, when that growth is work the BUYER must create and fund. Flag projections that capitalise buyer-created upside as the target's intrinsic value. (Overconfidence + Inside-View Dominance — the mirror of the operating-thesis overconfidence flagged above.)
- OWNER-DEPENDENCY MASKED AS TURNKEY — "absentee-ready / turnkey" where the seller's personal relationships, reputation, or tribal knowledge actually carry the revenue. Flag a turnkey claim with no documented management layer, key-customer-relationship map, or transition plan beneath it — this is the canonical post-close transition failure. (Inside-View Dominance, DI-B-022.)
- ADD-BACK INFLATION — "adjusted EBITDA" leaning on add-backs that recur (owner comp a hired GM replaces, "one-time" items that appear every year, unrealised run-rate). Flag aggressive add-backs that move the multiple; the SBA/lender will reject them and the DSCR math breaks at close. (Anchoring + Optimism — cross-reference the qofe overlay if a QofE is attached.)
- HOCKEY-STICK DETACHED FROM ACTUALS — a projection inflecting sharply upward with no mechanism tying it to the trailing actuals. Flag the discontinuity between historical and projected growth rates. (Planning Fallacy + Optimism.)
- DEFERRED-CAPEX-AS-MARGIN — high reported margins partly produced by under-investment (deferred maintenance, aging equipment, under-staffing). Flag "well-maintained / low-capex" claims with no capex schedule or asset-condition evidence — the deferred spend is a liability the buyer inherits at close. (Optimism + Sunk Ship if deal momentum is high.)
The screen's job is to convert a long seller narrative into a ranked list of the 3-5 claims to verify FIRST, so the searcher spends diligence cost only on targets that survive it.
