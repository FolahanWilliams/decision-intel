...
export async function assembleContext(request: IntelligenceRequest): Promise<IntelligenceContext> {
    ...
    const [newsResult, researchResult, caseStudyResult, macroResult, benchmarkResult] =
        await Promise.allSettled([
            ...
            biasTypes.length > 0
                ? utilTimeout(() => matchCaseStudies(biasTypes, industry, 5), 8_000, 'Case study lookup timeout')
    ...
    const research = extractResult<Record<string, ResearchPaper[]>>(researchResult, {}, 'research', errors);
    ...
    const industryBenchmarks = extractResult<IndustryBenchmark[]>(benchmarkResult, [], 'industryBenchmarks', errors);

    const durationMs = Date.now() - start;

    if (errors.length > 0) {
        log.warn(`Context assembly completed with ${errors.length} error(s) in ${durationMs}ms:`, errors);
    } else {
        log.info(`Context assembly complete in ${durationMs}ms — ` +
...
export function formatContextForPrompt(ctx: IntelligenceContext): string {
    ...
    if (ctx.news.length > 0) {
        const newsLines = ctx.news.slice(0, 5).map(n =>
    ...
    if (allPapers.length > 0) {
        const researchLines = allPapers.flatMap(([bias, papers]) =>
            papers.slice(0, 2).map(p => `- [${bias}] "${p.paperTitle}" (${p.year ?? 'n/a'}, cited ${p.citationCount}x)`)
        ).slice(0, 6);
    ...
    if (ctx.caseStudies.length > 0) {
        const caseLines = ctx.caseStudies.slice(0, 3).map(c =>
            `- ${c.company} (${c.year ?? 'n/a'}): ${c.outcome} — ${c.lessons.slice(0, 100)}`
        );
    ...
    if (ctx.industryBenchmarks.length > 0) {
        const benchLines = ctx.industryBenchmarks.slice(0, 4).map(b =>
            `- ${b.metric}: ${b.value} (${b.source})`
        );
