...
import { NEWS_FEEDS, ARTICLE_TTL_HOURS, FEED_FETCH_CONCURRENCY, type FeedConfig, type FeedCategory } from '@/config/newsFeeds';
...
async function fetchFeed(feed: FeedConfig): Promise<ParsedArticle[]> {
    try {
        ...
        log.warn(`Failed to fetch feed "${feed.name}": ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
}
...
async function classifyArticles(
...
): Promise<Array<ParsedArticle & { extractedTopics: string[]; biasTypes: string[]; industrySector: string | null; relevanceScore: number }>> {
    ...
    try {
        ...
        const classified: Array<ParsedArticle & { extractedTopics: string[]; biasTypes: string[]; industrySector: string | null; relevanceScore: number }> = [];
...
Return a JSON array of objects in the same order.`;
            ...
            try {
                ...
                const classifications = JSON.parse(text.replace(/```json\n?/g, '').replace(/```/g, '').trim());
                ...
                log.warn('Classification failed for batch, using defaults:', classErr instanceof Error ? classErr.message : String(classErr));
...
async function storeArticles(
    articles: Array<ParsedArticle & { extractedTopics: string[]; biasTypes: string[]; industrySector: string | null; relevanceScore: number }>
): Promise<number> {
    let added = 0;
    ...
    for (const article of articles) {
        try {
            ...
            if (code !== 'P2002') {
                log.warn(`Failed to store article "${article.title.slice(0, 50)}": ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
...
export async function syncAllFeeds(): Promise<SyncResult> {
    ...
    const allArticles = await batchProcess(
        NEWS_FEEDS,
        fetchFeed,
 ...
 */
export async function searchNews(options: NewsSearchOptions): Promise<Array<{
    id: string;
    title: string;
    link: string;
    source: string;
    ...
    const articles = await prisma.newsArticle.findMany({
        where: where as Parameters<typeof prisma.newsArticle.findMany>[0] extends { where?: infer W } ? W : never,
        orderBy: [
            { relevanceScore: 'desc' },
            { publishedAt: 'desc' },
        ],
