...
export class PdfGenerator {
  ...
  private drawScoreCard(
    ...
    if (textColor[0] === 255)
      this.doc.setTextColor(200, 200, 200); // Muted text on dark bg
  ...
  private addFactCheckTable(data: FactCheckData, startY: number): number {
    ...
    const tableData = data.verifications.map(v => [v.verdict, v.claim, v.explanation]);
  ...
  private addComplianceSection(compliance: ComplianceData) {
    ...
    this.doc.text(
      `Status: ${compliance.status}   |   Risk Score: ${compliance.riskScore}/100`,
      20,
      50
    );
  ...
  private addNoiseSection(
    noiseScore: number,
    stats: { mean: number; stdDev: number; variance: number }
  ) {
    this.doc.setFontSize(14);
    this.doc.setFont('helvetica', 'bold');
    this.doc.setTextColor(this.secondaryColor[0], this.secondaryColor[1], this.secondaryColor[2]);
    ...
    const finalY =
      (this.doc as jsPDF & { lastAutoTable: { finalY: number } }).lastAutoTable.finalY + 10;
    this.doc.text(
      'Lower noise = higher inter-judge agreement = more reliable analysis.',
      20,
      finalY
    );
  }
