...
describe('Integration Tests', () => {
  ...
  describe('verificationNode', () => {
    it('should extract tickers, fetch context, and verify claims', async () => {
                ...
                dataRequests: [
                  {
                    ticker: 'AAPL',
                    dataType: 'financials',
                    reason: 'Verify performance',
                    claimToVerify: 'Apple is doing well.',
                  },
                ],
                verifications: [
                  {
                    claim: 'Apple is doing well.',
                    verdict: 'UNVERIFIABLE',
                    explanation: 'Needs data',
                  },
                ],
              },
            }),
      ...
      (financialTools.getFinancialContext as unknown as ReturnType<typeof vi.fn>).mockResolvedValue(
        'AAPL Price: 150'
      );
    ...
    it('should handle no tickers found', async () => {
      ...
      mockGenerateContent.mockResolvedValueOnce({
        response: {
          text: () => JSON.stringify({ factCheck: { score: 100, flags: [], dataRequests: [] } }),
        },
      });
