...
// Pure types live in toxic-combinations-types.ts so the named-pattern
// catalogue (named-patterns.ts) can be imported by client components
// without webpack pulling in @prisma/client. Re-export here for back-
// compat with existing import paths.
export type { ContextFactors } from './toxic-combinations-types';
...
// The catalogue moved to named-patterns.ts so client components can
