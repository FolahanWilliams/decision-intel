...
export function buildIcpPromptBlock(): string {
  ...
  const beachhead = `BEACHHEAD FOCUS (the ETA / owner-operator wedge — locked 2026-06-26, the audit-as-LEVERAGE inversion): LEAD with ${leadLabel} (${BEACHHEAD_FOCUS.oneBuyer}). The ego-threat the gatekeeper-not-author lens worried about INVERTS here — these are authors, but the audit is fundraising leverage they WANT (they shop the memo to capital partners to raise). On a COLD first-touch, LEAD WITH THE RETRO though (Josh/Reiner 2026-06-29 correction, the $80M-claims reciprocity move): a cold prospect will NOT upload a LIVE CIM under LOE to a pre-revenue stranger, but a CLOSED deal carries zero confidentiality fear — open on two already-closed deals (one good, one that went sideways), free, and show what was flaggable in hindsight before anyone knew the outcome, NEVER "why your deal failed" (causation overclaim). The FORWARD live-deal audit (their fundraising-prep leverage) is the WARM follow-on ONCE the retro has shown it works. One buyer, one motion (${BEACHHEAD_FOCUS.oneMotion}), one proof (${BEACHHEAD_FOCUS.oneProof}). ${BEACHHEAD_FOCUS.notThis} CONTINUITY-CHASM DISCIPLINE (load-bearing): this wedge funds + proves + accumulates the Bias Genome data moat; it does NOT graduate to F500 (that is the Sankore mid-market BRIDGE). Keep the product governance-grade core; never build searcher-only capital-raising features. The EU AI Act Annex III ceiling moved to Dec 2027 (Digital Omnibus), giving an 18-month window to build the moat before the ceiling buyers are legally compelled.`;
