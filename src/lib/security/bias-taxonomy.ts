...
export const SECURITY_BIAS_TAXONOMY: BiasDefinition[] = [
  {
    ...
    securityContext:
      'Fixating on initial severity scores from security tools without considering context',
    triggers: [
  ...
  },
  {
    ...
    triggers: ['Prior similar incident', 'Team consensus on root cause', 'Vendor reputation'],
  ...
  },
  {
    ...
    triggers: ['New alert arrivals', 'Recent incident type', 'Latest threat intelligence'],
  ...
  },
  {
    ...
    triggers: ['Large prior investment', 'Long implementation time', 'Team training completed'],
  ...
  },
  {
    ...
    triggers: ['Vendor reputation', 'Previous tool success', 'Industry awards'],
  ...
  },
  {
    ...
    triggers: ['Industry trends', 'Peer company practices', 'Conference recommendations'],
    nudgeStrategy: 'Require environment-specific risk assessment',
    kpiImpact: ['Control Effectiveness', 'Resource Optimization'],
    detectionSignals: ['Trend-based decisions', 'No risk assessment', 'Copy-paste implementations'],
  },
  {
    ...
    triggers: ['Legacy system protection', 'Established procedures', 'Comfort with current tools'],
  ...
  },
  {
    ...
    triggers: ['Vendor severity labels', 'Alert description format', 'Dashboard presentations'],
  ...
  },
  {
    ...
    triggers: ['Limited experience', 'Early career success', 'Lack of feedback'],
  ...
  },
  {
    ...
    triggers: ['>100 alerts in queue', 'Multiple remediation options', 'Complex decision trees'],
...
export class SecurityBiasDetector {
  ...
  detectBias(decisionContext: {
    ...
    if (decisionContext.timeToDecision < 30 && decisionContext.dataPointsConsulted < 2) {
      results.push({
        ...
        nudgeRecommendation:
          'Consider additional context: Check identity permissions, data sensitivity, and network exposure before finalizing severity.',
    ...
    if (
      ...
      results.push({
        ...
        nudgeRecommendation:
          "Assign a devil's advocate: Have one team member explicitly argue against the current approach.",
    ...
    if (decisionContext.automationInvolved && decisionContext.dataPointsConsulted < 2) {
      results.push({
        ...
        confidence: 0.9,
        severity: 'critical',
        signals: ['Automated recommendation accepted', 'No manual verification performed'],
        nudgeRecommendation:
          'Verify automation: Review the reasoning behind the automated recommendation before execution.',
    ...
    if (decisionContext.productionSystem && decisionContext.timeToDecision > 300) {
      results.push({
        ...
        nudgeRecommendation:
          'Quantify risk: Compare potential breach cost ($X) vs temporary downtime cost ($Y).',
    ...
    if (decisionContext.alertVolume && decisionContext.alertVolume > 100) {
      results.push({
        ...
        nudgeRecommendation:
          'Simplify choices: Use smart filtering to show top 10 critical alerts first.',
  ...
  calculateCognitiveRisk(detections: BiasDetectionResult[]): {
    ...
    const totalScore = detections.reduce(
      (sum, d) => sum + severityWeights[d.severity] * d.confidence * 100,
      0
    );
    ...
    const primaryDetection = detections.reduce((max, d) =>
      severityWeights[d.severity] * d.confidence > severityWeights[max.severity] * max.confidence
        ? d
        : max
    );
    ...
    const mitigationPriority = detections
      .sort(
        (a, b) =>
          severityWeights[b.severity] * b.confidence - severityWeights[a.severity] * a.confidence
      )
      .slice(0, 3)
