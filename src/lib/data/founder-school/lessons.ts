...
// MATRIX + FRAMEWORK_COUNT derived too: the M-1 regression had these
// lessons teaching "20×20 / 30+ taxonomy / 17 frameworks" while the
// engine ran 22×22 and the registry had grown — a founder rehearsing
// investor answers from stale curriculum is the exact credibility hit.
    ...
    lessons: [
      {
        ...
        mnaPitch:
          'The diligence memos your team writes for IC are where DI earns its keep. Before a deal thesis reaches the partners, it runs through DI: confirmation bias, overconfidence, sunk-cost anchoring, base-rate neglect — each flagged with the exact sentence it lives in, and each carrying a historical exemplar from our 143-case library. Your deal team can say "we hit the same pattern Nortel did on Nokia-Siemens" with evidence, not analogy. You\'re not slowing deals down — you\'re removing the category of post-close partner question that starts with "why didn\'t we see that in Q3?"',
        corpStrategyPitch:
      ...
      },
      {
        ...
        order: 10,
        title:
          'The Specimen Audit Live: How to Run WeWork or Dangote in 7 Minutes Without Losing the Room',
        readTime: '6 min',
        summary:
      ...
      },
      {
        id: 'es_11',
        order: 10,
        title: 'Outcome Gate Enforcement as a Design-Partner Contractual Term',
        readTime: '5 min',
        summary:
          'The single move the NotebookLM strategic synthesis identified as the difference between "first paid conversion" and "month-6 ROI collapse." Make outcome capture mandatory, not optional, on every audited memo.',
        insight:
          'The pre-mortem question — "if Decision Intel fails to land its first paid design partner conversion in 6 months, what\'s the structural cause?" — produced an uncomfortable answer: the cathedral-of-code trap, with **outcome-gate avoidance** as the operational failure mode. The autopsy reads:\n\n(1) **Infinite Pilot Trap.** The free design partner asks for bespoke compliance / export features. You agree, because you\'re too meek to ask for money early on. 30 hours/week of unpaid dev shop work follows.\n\n(2) **Outcome Gate avoidance.** You shy from the interpersonal friction of telling the partner "your analysts MUST log the outcome 90 days from now." It feels like assigning homework.\n\n(3) **Month-6 ROI collapse.** Because you never forced outcome logging, the Causal AI flywheel has zero rotations. The DQI trend line at the renewal meeting is completely flat. The economic buyer refuses the £30K/year ask.\n\nThe structural fix is to make outcome capture **mandatory at the API level, not at the email-reminder level.** Per the locked 2026-04-26 hardening of `src/lib/learning/outcome-gate.ts` + `src/app/api/analyze/stream/route.ts`:\n\n- The Organization model carries an `enforceOutcomeGate Boolean @default(false)` field.\n- Free / individual / non-design-partner accounts: legacy soft-nudge experience. SOFT/HARD reminders fire as banners; analyses are never blocked.\n- Design-partner orgs (flag set to `true` as a contractual term of the partnership): when the user has 5+ pending outcomes past 30 days, `POST /api/analyze/stream` returns HTTP 409 with `code: "OUTCOME_GATE_BLOCKED"` and the pending analysis IDs. The user must log outcomes on past analyses before running a new audit.\n- The 7-minute live walk-through (`es_10`) sales motion is paired with the contractual ask: "we run all design partners with the outcome gate enforced — outcome reporting is part of the partnership, not optional. Your team logs every audit\'s outcome at 90 days; we recalibrate the DQI to your firm specifically. The flywheel is contractual, not aspirational."\n\n**The conversation in the design-partner discovery call:**\n\n> "Three things are non-negotiable in our partnership. First, the workflow mapping happens in this call so onboarding is 15 minutes, not 3 weeks. Second, your analysts run every IC memo through the audit before the partner meeting. Third, your analysts log the outcome 90 days after each decision — and Decision Intel\'s outcome gate enforces this at the platform level. If a partner tries to run a new audit while old ones are unresolved, the platform returns a block until the loop closes. We do this because the data flywheel is the entire reason this partnership compounds — without outcome capture, you\'re paying £2K/month for a bias detector. With outcome capture, you\'re paying £2K/month for a Decision Quality Index that recalibrates to your firm specifically over 12 months. The gate is the difference."\n\nMost design partners will accept this on the spot because (a) it makes the value proposition concrete, (b) it positions Decision Intel as a serious vendor with a procurement-grade discipline, not a startup vendor begging for engagement. The partners who refuse the gate are the ones who would have churned anyway in month 4 — better to surface that disagreement in the discovery call than at the renewal meeting.',
        whyItMatters:
          'NotebookLM Q6 synthesis (saved as note `9a249bd8` in the master KB): *"the deal will die not because your technology failed, but because you acted like an unpaid engineer trying to reach feature parity, rather than an enterprise CEO enforcing a strict, 30-day workflow change."* The outcome gate is the technical enforcement of the workflow change. Without it, every founder reflex (build a feature to please the user, send a softer reminder, give them more time) compounds into the no-rotation flywheel that fails the conversion. With it, the gate does the discipline so the founder doesn\'t have to be the heavy in every conversation.',
        action:
          "Three operational moves: (1) When approaching the next design-partner discovery call, lead with the three-part contractual ask (workflow mapping in-call, audit-before-partner-meeting, outcome-gate enforcement). Don't soften any of the three. (2) After contract signing, have an admin (or yourself, via direct DB update for now) flip `Organization.enforceOutcomeGate = true` for that org. The platform then enforces the discipline automatically. (3) When a design partner pushes back on the gate, the right response is \"this is what makes the partnership compound — partners who don't want this would have churned in month 4 anyway, so it's better that we're aligned now.\" If they still refuse, this isn't the right design partner.",
        reflection:
          "Of the three contractual asks (workflow mapping, audit-before-meeting, outcome gate), which one are you most likely to soften when the design partner pushes back? That's the one most likely to fail the conversion. Practise the response to that pushback before the call — by the time the partner asks, the answer needs to land in 10 seconds without hesitation.",
        csoPitch:
          "Three things are non-negotiable in our partnership. First, the workflow mapping happens in this call so onboarding is 15 minutes, not 3 weeks. Second, your analysts run every memo through the audit before the board pack is finalised. Third, your analysts log the outcome 90 days after each decision — Decision Intel's outcome gate enforces this at the platform level. The gate is the difference between a £30K-per-year subscription that compounds and a £30K-per-year subscription that dies at the renewal meeting because the DQI trend is flat. We make the data flywheel contractual, not aspirational, because that's what makes the partnership produce a lift you can show your audit committee.",
        mnaPitch:
          "For M&A design partners, the outcome gate is what turns the platform from a pre-IC bias-flagger into a deal-pattern recalibration engine specific to your firm. Every closed deal — win or loss — your analyst reports the outcome (revenue hit, revenue miss, integration flop, thesis held) within 90 days. The platform enforces this; if your team tries to run a new audit while past outcomes are unresolved, it returns a block until the loop closes. We do this because over 18-24 months of closed deals, your firm-specific DQI starts pointing at the failure modes your partners specifically under-weight in diligence. Without enforcement, you get a generic bias detector; with enforcement, you get a Causal AI tuned to YOUR firm's blind spots. The gate is the moat for your specific partnership.",
        vcPitch:
          'The outcome gate enforcement is the single change between the cathedral-of-code trap (the most likely failure mode at pre-seed for technical founders selling to enterprise) and the actual data flywheel. Most decision-tooling startups die because their "outcome reporting" is a polite email reminder users ignore. We made it contractual at the platform level for design-partner orgs — `Organization.enforceOutcomeGate = true` flips the API behaviour so new audits are blocked when old outcomes are unresolved. This forces the exact behaviour change that turns the product from "static audit tool" into "compounding decision performance OS." The discipline lives in code, not in the founder\'s willpower to chase reminders. That\'s the structural difference between selling decision intelligence and shipping it.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        order: 8,
        title:
          "Cold-Context On-Ramps: Plain Language That Doesn't Borrow Other People's Vocabulary",
        readTime: '4 min',
        summary:
          'In cold contexts, "the reasoning audit platform" doesn\'t land yet. The wrong on-ramp is "decision hygiene" — that\'s Kahneman\'s term, not ours. Pick a descriptive bridge that doesn\'t cede category language to a more famous author.',
        insight:
          'Decision Intel\'s locked primary vocabulary is "the reasoning audit platform" (locked 2026-05-04), "Recognition-Rigor Framework / R²F," and "Decision Quality Index / DQI" (per CLAUDE.md positioning lock). These are warm-context terms: they require the reader to already care enough to learn them. In cold contexts — a LinkedIn comment thread, a conference introduction, a cold email subject line — the reader has not yet invested. The temptation is to reach for "decision hygiene" as the plain-language on-ramp. Resist it. **"Decision hygiene" is Daniel Kahneman\'s phrase from his 2021 book "Noise"** (used to describe pre-decision debiasing protocols). Adopting it as our cold-context language means borrowing pre-existing vocabulary owned by a more famous author — every reader who recognises it associates the idea with Kahneman\'s book, not Decision Intel. That\'s the opposite of category creation; it\'s category contribution to someone else\'s shelf. Better cold-context on-ramps that stay descriptive without ceding ownership: (1) **"Strategic memo audits"** — artefact-specific, immediately concrete, no academic baggage. (2) **"Reasoning audit"** — softer variant of the locked claim, drops "platform" for cold contexts where SaaS-platform vocabulary feels heavy. (3) **"Pre-IC audit"** — workflow-specific for fund / M&A buyers; tells the reader exactly when in their process the product appears. (4) **"60-second audit on a strategic memo"** — the most empirical, evidence-first on-ramp; the literal product action. Once the reader engages, upgrade to the locked vocabulary in the next sentence.',
        whyItMatters:
          'Three failure modes to avoid: (a) Leading with "the reasoning audit platform" in a cold context can still lose the reader if the surface is too small (60-character LinkedIn DM subject) — drop "platform" and lead with "reasoning audit" or just "60-second audit on a strategic memo." (b) Leading with "decision hygiene" cedes category vocabulary to Kahneman — every recognition signal flows to "Noise," not to you. (c) Leading with "decision intelligence platform" puts you inside the Gartner crowd (Cloverpop, Aera, Peak.ai, Quantellia) per CLAUDE.md\'s explicit ban. The right move is descriptive plain language that names the artefact (memos), the work (audits), or the timing (pre-IC). All three are owned by no one and immediately intelligible to a CSO scrolling past at lunch.',
        action:
          'Write THREE versions of your 3-sentence product description: (a) cold LinkedIn comment opener using "60-second audit on a strategic memo," (b) cold email body using "pre-IC audit" or "reasoning audit" depending on whether the recipient is a fund / M&A or CSO buyer, (c) warm investor / second-meeting opener using "the reasoning audit platform" + "R²F" + "DQI" as the locked vocabulary. Practice the transition from cold to warm in two beats: descriptive sentence → category sentence. "We run 60-second audits on strategic memos. The category name is the reasoning audit platform — Recognition-Rigor Framework, scored as a Decision Quality Index." That\'s the bridge.',
        reflection:
        ...
        csoPitch:
          'In a cold email I\'ll write "60-second audit on a strategic memo" — descriptive, no jargon. In this room, with you already engaged, I\'ll use the precise vocabulary: the reasoning audit platform, R²F, DQI. The bridge between the two is one sentence — not a different product description. Cold-context language earns attention; warm-context language earns the category. The discipline matters because the reverse fails: leading cold with "the reasoning audit platform" loses the LinkedIn reader if the surface is too small, and leading warm with "decision hygiene" cedes our category to Kahneman.',
        vcPitch:
          'Category vocabulary discipline is a GTM asset most founders throw away. CLAUDE.md locks "the reasoning audit platform," "R²F," "DQI" as the warm-context claim — ownable, specific, defensible. Cold-context language uses descriptive plain phrases ("60-second audit on a strategic memo," "pre-IC audit") that are immediately intelligible without borrowing pre-existing academic vocabulary. We deliberately do NOT use "decision hygiene" because it\'s Kahneman\'s 2021 term — borrowing it would attribute every recognition signal to "Noise" instead of to us. The bridge from cold to warm is one sentence, practised against the advisor network. Every cold email earns one warm meeting; every warm meeting earns the locked vocabulary.',
      },
      {
        id: 'bd_9',
        order: 9,
  ...
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        vcPitch:
          "Pan-African geographic depth is the pre-seed moat that compounds. Two specimens (WeWork US, Dangote Pan-African), 19 frameworks (G7 / EU / GCC / African), sovereign-context branching for 10 specific EM jurisdictions in the structural-assumptions audit. Each new specimen, each new framework, each new sovereign branch widens the gap against a US-incumbent that would need 12-18 months of regulatory + cultural + specimen-library work to match this. The founder is Lagos-rooted, UK-resident, US-bound — tri-cultural lens that makes EM-focused fund partners pay attention in ways feature pitches don't. The moat isn't the AI; the AI is table stakes. The moat is the geography.",
      },
    ],
  },
  {
    ...
    lessons: [
      ...
      },
      {
        ...
        order: 5,
        title:
          "The Founder's Psychology · distress tolerance, emotional regulation, internal locus",
        readTime: '5 min',
        summary:
          "Weeks where nothing works are guaranteed. Solo founders need a research-backed protocol for that — there's no team to absorb the weight, and the macro environment (per the 2026-05-02 sociotechnical-convergence research) is materializing as the most volatile transition cycle in 80 years.",
        insight:
          "Refined 2026-05-02 with the sociotechnical-convergence research synthesis. Six tools that compound:\n\n(1) **Separate decision quality from outcome quality** — a good call that failed is still a good call. Brier scoring (which Decision Intel productizes for customers) is the same audit logic applied to your own decisions. Score the call BEFORE the outcome lands; revisit when the outcome arrives. The Outcome Gate flywheel in DI is the founder's personal calibration loop made into an enterprise-grade artefact.\n\n(2) **Weekly review ritual** — 20 min, same time, same day. 3 wins · 1 loss · 1 decision you'd make differently · 1 physiological signal (sleep / exercise / energy / sustained-focus capacity). The physiological column matters because the research paper's Reverse Flynn Effect findings show executive functioning degrades silently before performance does — track the input, not just the output.\n\n(3) **Distress tolerance as a deliberate input** — the Gen Z workforce data (Deloitte: 47% report stress / anxiety most or all the time; Gallup: 1.1-year average tenure) signals a generational deficit in the capacity to operate under friction. The defense is exposure, not avoidance. Specifics: cold exposure (2-3 min cold shower most mornings); physical exercise that progressively pushes effort tolerance; doing the hardest task of the day FIRST; deliberately presenting ideas to senior people (Wiz advisor / Mr. Gabe / Mr. Reiner network) who will push back. Each tolerated friction expands the capacity.\n\n(4) **Internal locus of control as the operating frame** (Rotter 1966) — distinct from positive thinking. The macro environment IS chaotic and structurally flawed (per the research paper's imperial-decline framework — US in Stage 5 by Dalio's model), but personal capacity to adapt, learn, and exert discipline remains entirely within control. Reject the prevailing generational narratives of victimization, nihilism, or inevitable doom. The macro is rigged AND the individual response determines the individual outcome. Both. Frame life as a strategic game of asset and skill allocation.\n\n(5) **The 10-year test** — will this matter in a decade? If not, don't give it more than 10 minutes of stress. This is a System 2 override on System 1 panic — most short-term setbacks (a lost prospect, a delayed feature, a critical comment) fail this test. The 10-year frame surfaces what actually compounds (relationships built, principles followed, protocols maintained) versus what doesn't.\n\n(6) **Physical reset before more thinking** — when the thinking loops, the answer is usually not more thinking. Cal Newport's deep-work protocol applies here: tolerance for boredom resets the dopaminergic baseline. A 30-min walk (no earbuds, no phone) often unlocks more synthesis than a 3-hour problem-staring session. The research paper specifically calls this out — algorithmic short-form video has trained the modern brain to expect instant stimulation; physical reset breaks the addiction loop.",
        whyItMatters:
          "At 16, you're building emotional regulation habits that will determine how you handle every future crisis — including the macro transition cycle the sociotechnical-convergence research projects through 2030. The founder who maintains clarity under pressure has a fundamental competitive advantage IF the cognitive baseline is protected (per Tier 1 of the Cognitive Sovereignty Stack visualized in the StrengthsWeaknessesMatrix). Without distress tolerance + internal locus, the upper-tier protocols (encoding, AI orchestration, founder-led marketing) collapse the moment friction arrives. This is the foundation under everything.",
        action:
          "Two protocols this week: (1) Set up the weekly 20-min review ritual with the FOUR columns (3 wins · 1 loss · 1 decision you'd make differently · 1 physiological signal). Same time, same day every week. Do it even in bad weeks — especially in bad weeks. (2) Pick ONE distress-tolerance input you'll add to the daily rhythm (cold shower, hardest-task-first, deliberate adversarial conversation with the Wiz advisor on a hard question). Track it for 30 days; the physiological column in the weekly review will show whether the input is producing the capacity.",
        reflection:
          "What's the emotion you're most likely to make bad decisions from? What's your specific protocol when you notice that state? And: when a peer says 'everything is rigged / AI is going to take all the jobs / it doesn't matter what we do' — recognize this as external-locus framing. The honest response: the macro is rigged, AND the individual response still determines the individual outcome. Both. Then redirect to action.",
      },
      {
        id: 'ldr_6',
        order: 6,
      ...
      },
      {
        id: 'ldr_7',
        order: 7,
        title: 'Learning Velocity · the personal R²F encoding mirror',
        readTime: '6 min',
        summary:
          "Your single biggest long-term advantage at 16 is learning velocity. You can compound it for 40 years. Refined 2026-05-02 with research-backed encoding protocols (Karpicke 2008 · Bjork 1994 · Tiago Forte · Wozniak SM-2) that mirror the platform's R²F architecture — the founder's personal-mind moat is isomorphic to the platform's decision-provenance moat, both fortified through the same capture → encode → recall loop.",
        insight:
          "Refined 2026-05-02 with the sociotechnical-convergence research synthesis (Karpicke 2008 · Bjork 1994 · Tiago Forte · Wozniak SM-2 · Cal Newport Deep Work · the Reverse Flynn Effect studies). The personal encoding stack mirrors the platform's R²F architecture. Five practices that compound, plus the four research-backed protocols that operationalise them:\n\n**Practice 1 · Deliberate reflection (decision journal)** — the same Outcome Gate flywheel Decision Intel runs on customer memos. Score the call BEFORE the outcome; revisit when it lands. Brier-scored over 100 entries, the founder's personal calibration converges toward the Tetlock superforecaster band.\n\n**Practice 2 · Exposure to exceptional people** — the Wiz advisor, Mr. Gabe, Mr. Reiner network, Sankore-class investors, future design partners. They compress years of learning into minutes. Cap: one or two genuine 1-hour conversations per quarter beats ten coffee chats.\n\n**Practice 3 · Deep reading + long-form content (NOT tweets, NOT short-form video)** — books and 30-90+ minute interviews train depth, patience, and synthesis. Algorithmic short-form (TikTok / Reels / Shorts) downregulates the Dorsolateral Prefrontal Cortex (DLPFC) and Anterior Cingulate Cortex (ACC) per fMRI/EEG studies cited in the sociotechnical-convergence research paper. Long-form does the opposite. Practical sources: Lex Fridman, The Knowledge Project (Shane Parrish), Acquired, founder interviews on YC channel, Howard Marks memos, BG2 (Brad Gerstner), All-In, primary papers from the master KB.\n\n**Practice 4 · Building in public** — teaching forces clarity of thought. Per GTM v3.3 §6 (the Sharran 1-1-1 lock), this is also the founder's ONE traffic source: 1× LinkedIn post per week anchored to a famous bias-resonance corporate decision (Kodak / Blockbuster / Nokia / WeWork / Theranos / Wirecard) from the 143-case library. Each post is a deliberate Active Recall + Elaborative Encoding pass — surfacing what was just learned to a public audience consolidates the encoding (Sparrow et al. 2011 cognitive offloading research is reversed when the learning is OUTPUTTED rather than retrieved from a database).\n\n**Practice 5 · Acting on uncertainty** — the best learning comes from doing. Distress tolerance (per ldr_5) is the load-bearing protocol; the friction of acting before you feel ready is what builds the capacity.\n\n**The four research-backed protocols (the personal R²F mirror — see PathToHundredMillion → R²F Deep Dive → 6th lever for the visualization):**\n\n*(a) Active Recall* (Karpicke & Roediger 2008, Science 319:966). Pause the long-form content / close the book. Retrieve the concept from memory in writing BEFORE checking. Re-reading FEELS more productive (fluent processing) but produces an illusion of mastery; recall PROVES mastery. 2-3× better long-term retention vs re-reading or concept-mapping.\n\n*(b) Elaborative Encoding* (Bjork & Bjork 1994 desirable-difficulty framework). After capturing, explain WHY the concept is true in your own words. Connect to existing knowledge — what does this remind you of in CLAUDE.md, in the product, in a prior decision? Three connections per concept beats ten flat facts.\n\n*(c) Progressive Summarization* (Tiago Forte, Building a Second Brain 2022). Layer 1 = raw notes / quotes during the source. Layer 2 = bold the most important sentences. Layer 3 = highlight the bold passages that survive a second read. Layer 4 = write a 2-3 sentence synthesis of just the highlights. Each layer is added on a separate pass, days or weeks apart. By Layer 4 the founder retrieves the highest-density 1-2% of the source in seconds.\n\n*(d) SM-2 spaced repetition* (Wozniak 1990 SuperMemo). Schedules reviews at exponentially widening intervals (Day 1 → Day 6 → multiplied by ~2.5× ease factor). Matches the forgetting curve. Education Room implements SM-2 lite — every drill of the learning_efficiency deck or buyer_personas deck reinforces the encoded knowledge at the optimal moment, not the convenient one.\n\n**The compounding loop:** these four protocols compose. Long-form consumption produces Layer-1 captures. Active Recall + Elaborative Encoding within 24h produces Layer-2 boldfacing. SM-2 review days later produces Layer-3 highlights. A monthly review produces Layer-4 synthesis. Over 12 months: the founder retrieves the highest-density 1-2% of every source consumed in seconds. Over 5 years: the founder becomes the single most informed operator on decision-intelligence in the GTM corridor. This is the personal moat-architecture mirror of what Decision Intel runs on a customer's memo (capture → encode → recall → outcome flywheel). The platform IS the protocol externalized.",
        whyItMatters:
          "The founders who reach unicorn scale are almost universally exceptional learners. It's not their initial knowledge — it's the rate at which they close the gap between where they are and where they need to be. You've already demonstrated this with Decision Intel. Systematize it WITH research-backed protocols, not generic 'read more.' The single biggest psychological lift the personal R²F mirror provides: when an investor or procurement reader asks how you stay current on a fast-moving category, the answer is structural ('I run Active Recall + Elaborative Encoding + Progressive Summarization on every long-form source, with SM-2 spacing — the same architecture Decision Intel runs on customer memos'), not aspirational ('I read a lot'). The founder demonstrates the protocol they sell.",
        action:
          'Three protocols this month (drill all three on the learning_efficiency deck in Education Room, then apply): (1) For the next long-form interview / paper / book — pause every 10-15 min and write down the 3 most important claims from memory BEFORE resuming. Active Recall in real time. (2) Within 24h of capture — write the Elaborative Encoding pass: WHY is this true, what does it remind me of in CLAUDE.md / product / prior decision? Three connections per concept. (3) On Day 6 + Week 4 + Month 3 of every captured source — run a Progressive Summarization pass. By Month 6 the synthesis is recallable in 15 seconds. The Education Room learning_efficiency deck (12 cards locked 2026-05-02) drills the protocol vocabulary; SM-2 spacing surfaces due cards on the optimal day.',
        reflection:
          "Who is one person that, if you had a 1-hour quarterly conversation with them, would most accelerate Decision Intel's success? How do you get that conversation? And: pick the next long-form source you will consume this week. Commit in writing to running the full Active Recall + Elaborative Encoding + Progressive Summarization protocol on it. Track the encoding quality on the learning_efficiency deck card review.",
      },
      {
        id: 'ldr_8',
        order: 8,
        title:
          'The University Application Is a Pitch — But Stanford Reads for Intellectual Vitality, Not Entrepreneurship',
        readTime: '6 min',
        summary:
    ...
    lessons: [
      {
        ...
        readTime: '6 min',
        summary: `Decision Intel's ${BIAS_COUNT}-bias taxonomy descends directly from Kahneman & Tversky's Nobel-winning research program. You should be able to trace every bias back to it.`,
      ...
      },
      {
        ...
        insight:
          'Kahneman, Sibony and Sunstein\'s "Noise: A Flaw in Human Judgment" (2021) is the sequel to "Thinking, Fast and Slow" and the intellectual foundation for our 3-frame noise measurement. Their thesis: in any system of professional judgment (underwriters, radiologists, judges, executives), if you show the same case to different experts, or the same expert on different days, you get wildly different answers. That variance is noise. Their field studies across insurance, law, medicine, and HR found noise typically accounts for as much error as bias — sometimes more. Decision Intel addresses noise directly: the pipeline runs three decorrelated samples over the same memo (analyst-skeptical / regulator-hostile / contrarian-strategist frames × two model architectures × random seed) and surfaces the standard deviation of their verdicts. The samples are decorrelated, not formally independent — LLMs share training data, so "independent" would be the wrong word per the GTM v3.3 honesty repair. A low-noise memo says what it means across all three frames; a high-noise memo is framing-sensitive and the disagreement tells you which audience will read it hardest.',
        whyItMatters:
        ...
        mnaPitch:
          "Ask three senior associates to read the same deal memo on a Monday morning and you'll get three different verdicts. Ask the same associate on a Tuesday versus a Friday and you'll often get two different verdicts. That variance is noise — and Kahneman's 2021 research showed it frequently accounts for more judgment error than bias does. DI runs three decorrelated samples over every memo (analyst-skeptical, regulator-hostile, contrarian-strategist frames across two model architectures) and reports the standard deviation of their verdicts. A noisy thesis is a warning to pressure-test before IC; a quiet thesis is a signal the reasoning is tight enough to defend.",
        corpStrategyPitch:
          'Kahneman\'s 2021 "Noise" book showed that professional judgment systems — underwriters, radiologists, judges, executives — produce wildly variable answers to identical cases, and that variance typically matches or exceeds bias as a source of error. Your strategic-review cadence runs on professional judgment. DI\'s 3-frame decorrelated-sample architecture (analyst-skeptical, regulator-hostile, contrarian-strategist lenses across two model architectures) measures how much the judgment on a specific recommendation would swing across equally qualified reviewers on different days. A high-noise memo is the one that needs another round of pressure-testing before the committee pack goes out. No other decision-support tool architecturally measures this.',
        vcPitch:
          "The bias-detection category is becoming commoditised. Noise measurement is not. Kahneman's 2021 book put noise on the map, and no incumbent — McKinsey, Cloverpop, or a ChatGPT prompt — architecturally measures it. Our 3-frame decorrelated-sample pipeline is a defensible product surface, not a marketing claim.",
      },
      {
        id: 'pf_3',
        order: 3,
        ...
        insight: `DQI is Decision Intel's single most-cited output. It grades a decision A–F on a 0–100 scale, boundaries A ≥ 85, B ≥ 70, C ≥ 55, D ≥ 40, F < 40. Under the hood it weights six components: (1) bias profile — presence and severity across the ${BIAS_COUNT}-bias taxonomy, (2) noise — cross-judge variance, (3) logical coherence — internal consistency of the argument, (4) evidence grounding — claims that survive fact-check, (5) pre-mortem robustness — how many failure modes the memo has considered, (6) stakeholder coverage — whether the dissenting voice is represented. The weights aren't arbitrary. Each comes from published research on what predicts good outcomes: noise and bias carry the most weight because Kahneman-Sibony showed they're the largest error sources; stakeholder coverage is weighted because Mercier & Sperber's argumentative theory of reasoning showed individual cognition is worst when uncontested. DQI is to strategic decisions what FICO is to credit: a transparent composite grounded in predictive research, not a vibe.`,
        whyItMatters:
        ...
        mnaPitch:
          "Every thesis gets a single-number DQI your IC can compare across deals — 28% bias profile, 18% noise, 18% logical coherence, 13% evidence grounding, 13% pre-mortem robustness, 10% stakeholder coverage. Partners who've sat through eight hours of diligence read-outs immediately get the value: one number, defensible weightings, pattern-matched against 143 historical theses. An A means this memo would have cleared IC at a firm that successfully acquired; a D means it shares structure with the ones that didn't. That's the signal gap we fill.",
        corpStrategyPitch:
      ...
      },
      {
        id: 'pf_6',
        order: 6,
        title: `The ${MATRIX} Bias Interaction Matrix — Why Biases Are Toxic in Combination`,
        readTime: '5 min',
        summary:
        ...
        insight: `The solo-bias literature (Tversky-Kahneman and descendants) treats each bias independently. The 2010s research asks the harder question: how do biases compound when they appear together? The answer is: multiplicatively, not additively. Confirmation bias paired with overconfidence produces catastrophic decision-making: you only look at evidence that supports your view (confirmation), and you trust that view more than the evidence warrants (overconfidence). You can predict the failure before it happens. Bazerman & Chugh (2022) "Better, Not Perfect" consolidates the interaction research; Kogut & Zander showed similar compounding effects in M&A contexts. Decision Intel's ${MATRIX} interaction matrix encodes 18 named toxic combinations from this literature — "overconfidence + confirmation," "sunk cost + optimism," "anchoring + availability" — each with a severity weight and a mapped historical case showing the pattern in the wild (Kodak, Blockbuster, Nokia, etc.). The compound DQI penalty for a toxic combination is larger than the sum of its parts.`,
        whyItMatters:
        ...
        mnaPitch: `The deals that fail catastrophically almost never fail from a single cognitive error. They fail from pattern: overconfidence plus confirmation plus sunk-cost anchoring is the exact triad that ran through AOL-Time Warner, HP-Autonomy, and Kraft-Heinz-Unilever. DI's ${MATRIX} interaction matrix flags 18 named toxic combinations and maps each to the specific historical deal that demonstrated the pattern. Your associate's memo doesn't get "confirmation bias detected"; it gets "this is structurally the Kraft-Heinz pattern, here's the post-close literature on what broke and why."`,
        corpStrategyPitch: `The strategic-planning decisions that unwind in hindsight almost always carry a toxic combination — not a single bias. Kodak's 2012 collapse was confirmation plus overconfidence plus sunk cost compounding over a decade. DI's ${MATRIX} matrix encodes 18 of these named patterns with the historical exemplar for each, so when a recommendation carries the Kodak pattern, the flag doesn't read as an algorithmic verdict — it reads as "your memo's structural profile matches the company-level decision sequence that preceded a known multi-billion-dollar unwind." That's the conversation steering committees can actually have.`,
        vcPitch:
      ...
      },
      {
        ...
        insight: `Every memo goes through 12 nodes in our LangGraph pipeline. They split into roughly 8 sequential + 4 parallel, driven by data dependencies. The sequence: (1) GDPR / NDPR anonymiser strips PII, (2) structurer parses the memo into sections, (3) intelligence gatherer pulls relevant external context, (4) bias detection flags the ${BIAS_COUNT}-bias taxonomy matches, (5) three-judge noise measurement runs in parallel, (6) logical coherence analysis checks argument validity, (7) pre-mortem generates failure scenarios, (8) Red Team surfaces the top adversarial objection, (9) fact-check verifies quantitative claims against external sources, (10) compliance mapper cross-links flags to ${FRAMEWORK_COUNT} regulatory frameworks across G7 / EU / GCC / African markets, (11) compound-risk scorer applies the ${MATRIX} matrix, (12) verdict synthesiser produces DQI + summary. Each node has a specialised prompt and a specialised output schema; the outputs feed forward. A single LLM call cannot hit this depth because the model has to do everything at once with no specialised context per step. The pipeline is the product. It's also why per-audit cost is ~£0.30-0.50 (~$0.40-0.65) — we're firing ~17 LLM calls under the locked 2-model policy (gemini-3-flash-preview + gemini-3.1-flash-lite), not one.`,
        whyItMatters:
      ...
      },
      {
        id: 'pf_9',
        order: 9,
        title: 'The 143 Case Library — Cross-Decision Intelligence',
        readTime: '6 min',
        summary:
          "Our library of 143 audited historical decisions is not a marketing asset — it's the training prior for every new audit. Pattern-matching a new memo against known failures is the research contribution.",
        insight:
          'Our library currently holds 143 deduplicated historical strategic decisions — Kodak digital delay, Blockbuster rejecting Netflix, Nokia on touchscreen, Kraft-Heinz 3G merger, Wirecard fraud, Theranos, Quibi, Meta metaverse pivot, etc. Each case carries the pre-decision memo (or reconstructed equivalent), the biases present at decision time, the red flags detectable without hindsight, and the eventual outcome. Tier 2 cases carry DQI estimate, timeline, stakeholders, counterfactual, post-mortem citations, and pattern-family classification. The library powers two distinct product behaviours: (1) Bias Genome — a ranked leaderboard of which biases appear most often in failed decisions, with failure-lift multipliers computed against baseline, and (2) cross-decision intelligence on the audit page — "this memo\'s bias profile matches 12 historical cases; 9 of 12 produced negative outcomes." Number matters: 143 is large enough to reject "anecdotal" objections, small enough that each entry is hand-verified and primary-source cited. We deliberately fight survivorship bias by including cases where the same pattern produced good outcomes.',
        whyItMatters:
        ...
        reflection:
          'If a VC said "143 cases is small — McKinsey has thousands," what is your two-sentence answer? If you don\'t have one, write it now.',
        sources: [
          {
            label: 'Internal',
            detail:
              'src/lib/data/case-studies/* — the full 143-case library, deduplicated and primary-source cited.',
          },
          {
