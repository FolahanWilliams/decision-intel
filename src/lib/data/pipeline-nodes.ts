...
export const PIPELINE_NODES: PipelineNode[] = [
  // ─── Preprocessing (sequential) ────────────────────────────────────
  {
    ...
    output: 'A cleaned document with structured redaction markers, plus an anonymization status flag.',
    academicAnchor: 'GDPR Art. 5(1)(c) data minimisation — treated as a hard prerequisite, not a postprocessing step.',
  },
  {
    id: 'structurer',
    zone: 'preprocessing',
  ...
  },
  {
    ...
    output: 'Shared intelligence context: topic, industry, prior related decisions, sector benchmarks.',
    academicAnchor: 'Retrieval-augmented reasoning — grounds every downstream judgement in organization history.',
  },
  ...
  // ─── Analysis (parallel fan-out) ───────────────────────────────────
  {
    ...
    output: 'A list of bias instances: { biasType, severity, confidence, excerpt, explanation, suggestion }.',
    academicAnchor: 'Kahneman & Tversky, Heuristics and Biases (1974); plus investment-specific extensions from Strebulaev.',
  },
  {
    id: 'noiseJudge',
    zone: 'analysis',
  ...
  },
  {
    ...
    output: 'Verification verdicts (VERIFIED / CONTRADICTED / UNVERIFIABLE) and a compliance exposure report.',
  ...
  },
  {
    ...
    academicAnchor: 'Helmer, 7 Powers; Porter, Five Forces; classical informal-logic fallacy literature.',
  },
  {
    id: 'simulationNode',
    zone: 'analysis',
    ...
    output: 'An overall verdict, individual twin votes (APPROVE / REJECT / REVISE), and a dissent count.',
  ...
  // ─── Synthesis (sequential) ────────────────────────────────────────
  {
    ...
    academicAnchor: 'Tetlock & Gardner — aggregation of diverse judgement outperforms any single-model output.',
  },
  {
    id: 'riskScorer',
    zone: 'synthesis',
    ...
    output: 'The Decision Quality Index (0–100, A–F grade), toxic combination flags, and a calibrated report.',
    academicAnchor: 'Bayesian decision theory + Pearl\'s do-calculus for counterfactual projections.',
  },
];
...
export const ZONES: Record<PipelineZone, { label: string; description: string }> = {
  ...
  analysis: {
    label: 'Analysis',
    description: 'Parallel fan-out — seven specialized agents reason over the same shared context simultaneously.',
  },
  synthesis: {
    label: 'Synthesis',
