...
Return a JSON array.`;
        ...
        const papers = JSON.parse(text.replace(/```json\n?/g, '').replace(/```/g, '').trim());
 ...
 */
export async function findResearchForBias(biasType: string, limit: number = 5): Promise<ResearchPaper[]> {
    // Check cache first
    ...
    cachePapers(biasType, topPapers).catch(
        err => log.warn('Failed to cache research papers:', err)
    );
