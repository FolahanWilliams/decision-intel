...
function ArticleCard({ article, delay }: { article: NewsArticle; delay: number }) {
  ...
  return (
    <div className="card animate-slide-up" style={{ animationDelay: `${delay}s`, padding: '16px' }}>
      <div
        style={{
          display: 'flex',
...
export default function IntelligencePage() {
  ...
  const handleSync = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSyncMessage(
          `Synced ${data.feedsProcessed} feeds, ${data.articlesAdded} new articles in ${(data.durationMs / 1000).toFixed(1)}s`
        );
        mutateStatus();
        mutateNews();
      } else {
  ...
  return (
    ...
    >
      <Breadcrumbs
        items={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Intelligence' }]}
      />
        ...
        >
          ...
          <StatCard
            ...
            value={
              statusLoading
                ? '...'
                : status?.hoursOld === Infinity
                  ? 'N/A'
                  : status?.hoursOld !== undefined && status.hoursOld < 1
                    ? '<1h'
                    : `${Math.round(status?.hoursOld ?? 0)}h`
            }
          ...
          {categories.map(cat => (
            <button
              ...
              style={{
                ...
                fontWeight: 500,
                background:
                  categoryFilter === cat ? 'var(--accent-primary)' : 'var(--bg-secondary)',
...
function MacroSection() {
  const [macro, setMacro] = useState<{
    indicators: Array<{ name: string; value: string; period: string; source: string }>;
    summary: string;
  } | null>(null);
  ...
  if (error) {
    return (
      <div className="card" style={{ padding: '24px', textAlign: 'center' }}>
        ...
        <button onClick={loadMacro} className="btn" style={{ fontSize: '12px' }}>
          Retry
        </button>
      </div>
    );
