...
const PAGE_TABS = [
  { key: 'recordings', label: 'Recordings', icon: <Video size={15} /> },
                        ...
                        <td style={{ textAlign: 'center', padding: '14px 8px' }}>
                          {(() => {
                            ...
                            const decisions = Array.isArray(meeting.keyDecisions)
                              ...
                              : [];
                            const actionTotal = actions.length + decisions.length;
                            if (actionTotal > 0) {
                              return (
                                <div
...
function MeetingsInner() {
  ...
  const tab = searchParams.get('tab') === 'rooms' ? 'rooms' : 'recordings';
    ...
    <div>
      <div className="container" style={{ paddingTop: 'var(--spacing-2xl)', paddingBottom: 0 }}>
        ...
        <TabBar
          tabs={PAGE_TABS}
          activeTab={tab}
          onTabChange={key => router.replace(`/dashboard/meetings?tab=${key}`, { scroll: false })}
        />
      </div>
      <div
        className="container"
        style={{ paddingTop: 'var(--spacing-md)', paddingBottom: 'var(--spacing-2xl)' }}
      >
        {tab === 'recordings' ? <RecordingsContent /> : <DecisionRoomsContent />}
      </div>
    </div>
  );
}
