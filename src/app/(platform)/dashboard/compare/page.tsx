...
export default function ComparePage() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      const docs: ComparisonDoc[] = results.filter(Boolean).map(r => {
  ...
  const metrics = [
    {
      ...
      format: (v: number | null) => (v != null ? `${v}/100` : '—'),
      higherBetter: true,
    },
    {
      key: 'noiseScore',
    ...
    <div style={{ padding: 'var(--spacing-xl)', maxWidth: 1200, margin: '0 auto' }}>
      ...
      {selectedIds.length < 2 && (
        ...
        >
          <GitCompareArrows
            size={40}
            style={{ color: 'var(--text-muted)', opacity: 0.4, marginBottom: 'var(--spacing-md)' }}
          />
        ...
        <div className="card liquid-glass-premium" style={{ marginBottom: 'var(--spacing-xl)' }}>
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              ...
              <tbody>
                {metrics.map(metric => {
                  const values = comparisonDocs.map(
                    d => (d as unknown as Record<string, unknown>)[metric.key]
                  );
                  ...
                  return (
                    <tr key={metric.key}>
                      ...
                      {comparisonDocs.map(doc => {
                        ...
                        const isBest =
                          typeof val === 'number' && val === best && numericValues.length > 1;
                        return (
                          <td
                            key={doc.id}
                            style={{
                              ...
                              color: isBest ? 'var(--accent-primary)' : 'var(--text-primary)',
                            }}
                          >
        ...
        <div style={{ marginBottom: 'var(--spacing-xl)' }}>
          ...
          {biasAnalysis.shared.length > 0 && (
            <div style={{ marginBottom: 'var(--spacing-md)' }}>
              <div
                className="flex items-center gap-xs"
                style={{ fontSize: '12px', color: 'var(--warning)', marginBottom: '8px' }}
              >
                <AlertTriangle size={14} />
                <span style={{ fontWeight: 600 }}>
                  Shared Biases ({biasAnalysis.shared.length})
                </span>
              </div>
