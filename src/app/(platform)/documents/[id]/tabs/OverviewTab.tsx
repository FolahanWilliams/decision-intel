          ...
          {biases.length === 0 ? (
            ...
            <div className="space-y-4">
              {biases.map((bias, i) => (
                <div
                  key={i}
                  className={`p-4 border bg-card/50 ${bias.severity === 'critical' ? 'border-red-500/20 bg-red-500/5' : 'border-border'}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      <span
                        className={`text-xs font-bold uppercase px-2 py-0.5 ${bias.severity === 'critical' ? 'bg-red-500/20 text-red-400' : 'bg-slate-700 text-slate-300'}`}
                      >
                        {bias.biasType}
                      </span>
                    </div>
