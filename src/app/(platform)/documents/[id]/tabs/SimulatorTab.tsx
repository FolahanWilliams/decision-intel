  ...
  return (
    <>
      ...
      {simulationResult && (
        <div
          className="card"
          style={{ borderColor: 'var(--accent-secondary)', background: 'rgba(10, 132, 255, 0.03)' }}
        >
          <div
            className="card-header justify-between"
          ...
          <div className="card-body" style={{ padding: 'var(--spacing-lg)' }}>
            ...
            >
              <div style={{ textAlign: 'center' }}>
                ...
                <div
                  style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--text-secondary)' }}
                >
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                >
                  {simulationResult.overallScore > (originalScore || 0) ? '+' : ''}
                  {Math.round(simulationResult.overallScore - (originalScore || 0))} pts
                </div>
              </div>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: '10px', color: 'var(--text-muted)', marginBottom: '4px' }}>
                ...
                <div
                  style={{
                    ...
                    fontWeight: 800,
                    color:
                      simulationResult.overallScore >= 70
                        ? 'var(--success)'
                        : simulationResult.overallScore >= 40
                          ? 'var(--warning)'
                          : 'var(--error)',
                  }}
                >
                ...
                >
                  ...
                  <tbody>
                    <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      >
                        {simulationResult.overallScore > (originalScore || 0) ? '↑' : '↓'}{' '}
                        {Math.abs(Math.round(simulationResult.overallScore - (originalScore || 0)))}
                      </td>
                    </tr>
                    <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                      ...
                      <td
                        style={{
                          ...
                          fontWeight: 600,
                          color:
                            simBiasCount < originalBiasCount
                              ? 'var(--success)'
                              : simBiasCount > originalBiasCount
                                ? 'var(--error)'
                                : 'var(--text-muted)',
                      ...
                      >
                        {simBiasCount < originalBiasCount
                          ...
                          : simBiasCount > originalBiasCount
                            ? '↑'
                            : '='}{' '}
                        {Math.abs(simBiasCount - originalBiasCount)}
                      </td>
                    </tr>
                    <tr style={{ borderBottom: '1px solid var(--border-color)' }}>
                      <td style={{ padding: '12px 0', fontSize: '13px' }}>Noise Level</td>
                      <td style={{ textAlign: 'center', fontWeight: 600 }}>
                        {originalNoiseScore != null ? Math.round(originalNoiseScore) : '--'}%
                      </td>
                      <td style={{ textAlign: 'center', fontWeight: 600 }}>
                        {Math.round(simulationResult.noiseScore)}%
                      </td>
                      <td
                        style={{
                          textAlign: 'center',
                          fontWeight: 600,
                          color:
                            simulationResult.noiseScore < (originalNoiseScore || 0)
                              ? 'var(--success)'
                              : 'var(--error)',
                        }}
                      >
                        {simulationResult.noiseScore < (originalNoiseScore || 0) ? '↓' : '↑'}{' '}
                        {Math.abs(
                          Math.round(simulationResult.noiseScore - (originalNoiseScore || 0))
                        )}
                        %
                      </td>
                    </tr>
      ...
      {simulationResult &&
        (originalBiasTypes.length > 0 || simulationResult.biases.length > 0) &&
        (() => {
          const simBiasTypes = new Set(
            simulationResult.biases.map((b: BiasInstance) => b.biasType)
          );
          ...
          const newBiases = simulationResult.biases.filter(
            (b: BiasInstance) => !origBiasTypes.has(b.biasType)
          );
          ...
          return (
            <div>
              ...
              >
                Bias Analysis — {resolved.length} resolved, {stillPresent.length} remaining
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                {allItems.length > 8 && (
                  <div
                    style={{ fontSize: '11px', color: 'var(--text-muted)', textAlign: 'center' }}
                  >
      ...
      {simulationResult && (
        ...
        >
          ...
          <div style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>
            {(() => {
              const resolvedCount = originalBiasTypes.filter(
                t => !simulationResult.biases.some((b: BiasInstance) => b.biasType === t)
              ).length;
