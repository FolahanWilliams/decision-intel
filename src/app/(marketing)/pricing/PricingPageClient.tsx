...
const FAQ: Array<{ q: string; a: string }> = [
  ...
  {
    q: 'Is the Decision Quality Index just a black-box grade I can’t defend to my board?',
    a: 'No. The DQI is a reproducible weighted methodology (version 2.4.0), not an opaque grade. Three things make it defensible: (1) every score decomposes into seven named components, each with a per-component breakdown showing exactly which reasoning signals drove the number — nothing is hidden behind a single figure; (2) the component weights are user-adjustable, so your team tunes them to your domain (this is the documented fix for algorithm aversion — Dietvorst 2016: people trust an imperfect model they can adjust); (3) every score carries a tamper-evident hash on the Decision Provenance Record, so the same memo produces the same number, verifiably, for anyone who re-runs it. It is a baseline you shape and can defend, not a verdict imposed on you.',
  },
  {
