...
export async function GET(request: NextRequest) {
    ...
    try {
        ...
        // 1. Sync RSS feeds
        let syncResult = { feedsProcessed: 0, articlesAdded: 0, articlesExpired: 0, errors: [] as string[], durationMs: 0 };
        try {
        ...
        try {
            ...
            log.warn('Failed to log sync metadata (schema drift?):', dbErr instanceof Error ? dbErr.message : String(dbErr));
        }

        log.info(`Intelligence sync complete in ${durationMs}ms: ` +
        ...
        return NextResponse.json(
            { error: 'Intelligence sync failed', details: error instanceof Error ? error.message : String(error) },
            { status: 500 }
        );
    }
}
