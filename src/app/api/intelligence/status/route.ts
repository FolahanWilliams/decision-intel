...
export async function GET() {
    try {
        ...
        const [
            ...
            prisma.intelligenceSync.findFirst({
                orderBy: { createdAt: 'desc' },
                select: { createdAt: true, status: true, articlesAdded: true, feedsProcessed: true, durationMs: true },
            }).catch(() => null),
        ...
        const hoursOld = lastDataAt ? (now.getTime() - new Date(lastDataAt).getTime()) / (1000 * 60 * 60) : Infinity;
