...
function buildSystemPrompt(
    ragContext: Array<{ filename: string; content: string; similarity: number; score: number; biases: string[] }>
): string {
    ...
    try {
        ...
        const chat = model.startChat({
            history: [
                ...
                { role: 'model', parts: [{ text: 'Understood. I will answer questions based on the retrieved document analyses and follow the rules provided.' }] },
                ...geminiHistory,
            ],
        });
        ...
        const stream = new ReadableStream({
            async start(controller) {
                try {
                    // Send sources first
                    controller.enqueue(
                        encoder.encode(formatSSE({ type: 'sources', sources }))
                    );
                    ...
                    for await (const chunk of result.stream) {
                        ...
                        if (text) {
                            controller.enqueue(
                                encoder.encode(formatSSE({ type: 'chunk', text }))
                            );
                        }
                    }
