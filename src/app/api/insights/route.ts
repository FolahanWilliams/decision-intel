...
export async function GET() {
  try {
    ...
    const [analyses, biasDistribution, severityCounts] = await Promise.all([
    ...
    for (const a of analyses as any[]) {
      ...
      // Compliance
      const comp = a.compliance as {
        riskScore?: number;
        regulations?: { name?: string; status?: string }[];
      } | null;
      if (comp?.riskScore != null) {
        totalCompliance += 100 - comp.riskScore;
        complianceCount++;
      }
      if (comp?.regulations) {
      ...
      if (swot) {
        for (const s of swot.strengths || [])
          swotAgg.strengths.set(s, (swotAgg.strengths.get(s) || 0) + 1);
        for (const w of swot.weaknesses || [])
          swotAgg.weaknesses.set(w, (swotAgg.weaknesses.get(w) || 0) + 1);
        for (const o of swot.opportunities || [])
          swotAgg.opportunities.set(o, (swotAgg.opportunities.get(o) || 0) + 1);
        for (const t of swot.threats || [])
          swotAgg.threats.set(t, (swotAgg.threats.get(t) || 0) + 1);
      }

      // Logic fallacies
      const laFull = a.logicalAnalysis as {
        score?: number;
        fallacies?: { name?: string; severity?: string }[];
      } | null;
      if (laFull?.fallacies) {
        ...
        for (const f of laFull.fallacies) {
          ...
          if (existing) {
            existing.count++;
            if ((severityOrder[sev] ?? 0) > (severityOrder[existing.severity] ?? 0))
      ...
      const cog = a.cognitiveAnalysis as {
        blindSpotGap?: number;
        blindSpots?: { name?: string }[];
      } | null;
      if (cog?.blindSpotGap != null) {
        totalBlindSpotGap += cog.blindSpotGap;
        blindSpotGapCount++;
      }
      if (cog?.blindSpots) {
    ...
    const topBlindSpots = [...blindSpotFreq.entries()]
      ...
      .slice(0, 5)
      .map(([name]) => name);
    const avgBlindSpotGap =
      blindSpotGapCount > 0 ? Math.round(totalBlindSpotGap / blindSpotGapCount) : 0;

    // Derived: twin vote totals
    const decisionTwinVotes = {
      ...twinVotes,
      total: twinVotes.approve + twinVotes.reject + twinVotes.revise,
    };
    ...
    const payload = {
      ...
      // Summary stats
      totalAnalyses: n,
      totalBiases: biasDistribution.reduce(
        (sum: number, b: { count: bigint }) => sum + Number(b.count),
        0
      ),
    };
