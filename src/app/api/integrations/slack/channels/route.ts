...
export async function GET() {
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: 'Token decryption failed — try reconnecting Slack' }, { status: 500 });
    }

    // Fetch channels from Slack API (bot must be a member)
    const slackRes = await fetch('https://slack.com/api/conversations.list?' + new URLSearchParams({
    ...
    if (!slackData.ok) {
      ...
      if (slackData.error === 'token_revoked' || slackData.error === 'token_expired' || slackData.error === 'invalid_auth') {
        return NextResponse.json({ error: 'Slack token expired — please reconnect' }, { status: 401 });
      }

      return NextResponse.json({ error: 'Failed to fetch channels from Slack' }, { status: 502 });
    }

    const channels = (slackData.channels || []).map((ch: {
      id: string;
      name: string;
      is_private: boolean;
      num_members: number;
