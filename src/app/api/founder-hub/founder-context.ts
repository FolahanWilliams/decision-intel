...
Pricing: Starter (Free, 4 analyses) → Corp Dev ($2,499/mo, unlimited) → Enterprise (Custom). ALSO: Per-Deal Audit ($4,999 one-time, flat rate). Grants unlimited analyses for all documents linked to that deal. Price lever: per-deal bypasses procurement — a VP can expense $4,999 on a single deal, then convert to subscription once value is proven.
...
Beat 12 — BUSINESS MODEL: "SaaS subscription (Starter free, Corp Dev $2,499/mo, Enterprise custom) PLUS per-deal audit ($4,999 one-time, flat rate). Per-deal bypasses procurement — expense it on the deal. 97% gross margins at $0.03-0.07 API cost. Corp dev VP can approve $2,499/mo without a procurement cycle."
...
Beat 14 — UNIT ECONOMICS: "~97% gross margins. $0.03-0.07 API cost per analysis. Per-deal audit at $4,999 on any deal is a rounding error for the buyer and pure margin for us. CAC for corporate M&A is relationship-driven (advisor network) and accessible via compliance/risk channels."
...
Step 7 DECISION SPEED: Per-deal audit ($4,999 one-time) bypasses procurement. A VP can expense it on a single deal. Subscription ($2,499/mo) requires slightly longer cycle but per-deal is the wedge — prove value, then convert.
