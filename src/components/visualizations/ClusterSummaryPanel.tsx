...
export function ClusterSummaryPanel({
  ...
  const failureRate =
    stats.outcomes.success + stats.outcomes.failure > 0
      ? Math.round(
          (stats.outcomes.failure / (stats.outcomes.success + stats.outcomes.failure)) * 100
        )
      : 0;
    ...
    <div className="card" style={{ marginBottom: 'var(--spacing-md)' }}>
      <div className="card-header flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-zinc-300">Cluster Analysis</span>
      ...
      <div className="card-body">
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          ...
          <StatBox
            label="Success"
            value={stats.outcomes.success}
            icon={<CheckCircle size={12} />}
            color="#22c55e"
          />
          <StatBox
            label="Failure"
            value={stats.outcomes.failure}
            icon={<AlertTriangle size={12} />}
            color="#ef4444"
          />
          <StatBox
            label="Pending"
            value={stats.outcomes.pending}
            icon={<Clock size={12} />}
            color="#71717a"
          />
        </div>
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              {stats.sharedParticipants.map(([name, count]) => (
                <span
                  key={name}
                  className="px-1.5 py-0.5 bg-teal-500/10 text-teal-400 rounded text-[10px]"
                >
                  {name} ({count})
                </span>
              ))}
