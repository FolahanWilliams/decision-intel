...
export function BiasNetwork({ biases = [] }: BiasNetworkProps) {
  ...
  if (nodes.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 text-muted">No bias data available</div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <svg viewBox="0 0 400 400" className="w-full h-full max-w-[500px] max-h-[500px]">
          ...
          {nodes.map(node => {
            ...
            const isDimmed =
              ...
              !connections.some(
                c =>
                  (c.from === selectedNodeId && c.to === node.id) ||
                  (c.from === node.id && c.to === selectedNodeId)
              );
            ...
            return (
              ...
              >
                ...
                >
                  {node.name.length > 20 && !isSelected
      ...
      {/* Details Panel */}
      <div
        className={`mt-4 transition-all duration-300 ${selectedNodeId ? 'opacity-100 max-h-48' : 'opacity-50 max-h-12 overflow-hidden'}`}
      >
        {selectedNode ? (
          <div className="p-4 bg-secondary/40 border border-border">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-semibold text-lg flex items-center gap-2">
                <span
                  className="w-3 h-3"
                  style={{ backgroundColor: severityColors[selectedNode.severity] }}
                />
                {selectedNode.name}
              </h4>
              <span
                className="text-xs px-2 py-0.5 border border-white/10 uppercase"
                style={{ color: categoryColors[selectedNode.category] }}
              >
          ...
          <div className="flex flex-wrap gap-4 justify-center text-xs mt-2 border-t border-white/5 pt-4">
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 bg-success/20 border border-success" />{' '}
              <span className="text-muted">Low</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 bg-warning/20 border border-warning" />{' '}
              <span className="text-muted">Medium</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-3 bg-error/20 border border-error" />{' '}
            ...
            <div className="flex flex-wrap gap-2">
              {categoryBiases.map((bias, idx) => (
                <span
                  ...
                  style={{
                    borderColor:
                      severityColors[bias.severity as keyof typeof severityColors] + '40',
