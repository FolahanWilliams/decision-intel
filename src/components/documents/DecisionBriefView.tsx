...
function rankBias(b: BiasInstance): number {
...
function deriveDocumentType(documentType: string | null | undefined): string {
  ...
  return 'Strategic memo';
}

/** Estimate the cost-of-ignoring from exposure + bias severity profile.
 *  Per NotebookLM Q2 synthesis: this is the single most leveraged number
 *  on the hook line. When DecisionFrame.value is unavailable, use the
 *  generic "Likely material" framing instead of fabricating a number. */
function buildCostOfIgnoring(
  ...
  // Severity-weighted percentage of exposure
  const severityWeight = topBiases.reduce(
    (sum, b) => sum + (SEVERITY_RANK[b.severity?.toLowerCase()] ?? 0),
    0
  );
  // Map weight 0..12 → 5%..40% expected loss exposure
