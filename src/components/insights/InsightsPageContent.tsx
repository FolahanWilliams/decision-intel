...
export function InsightsPageContent() {
  ...
  const statCards = [
    {
      ...
      icon: <Brain size={16} />,
      color:
        insights.radar.quality >= 70
          ? 'var(--success)'
          : insights.radar.quality >= 50
            ? 'var(--warning)'
            : 'var(--error)',
      suffix: '/100',
    ...
    },
    {
      ...
      color:
        insights.totalBiases > 10
          ? 'var(--error)'
          : insights.totalBiases > 5
            ? 'var(--warning)'
            : 'var(--success)',
    },
    {
  ...
  return (
    ...
    >
      ...
      {/* ── [02] PERFORMANCE TRAJECTORY (merged w/ historical trends) ─ */}
      ...
      <div className="animate-slide-up" style={{ animationDelay: '0.28s' }}>
            ...
            {trendData && trendData.trendData.length > 0 && (
              <button
                ...
                onClick={() => {
                  ...
                  a.href = url;
                  a.download = `trajectory-${trendRange}.csv`;
                  a.click();
        ...
        <div className="card card-glow" style={{ marginBottom: 'var(--spacing-md)' }}>
          <div className="card-header flex items-center justify-between">
            ...
            {trendData && trendData.stats && (
              <div
                style={{
                  fontSize: '10px',
                  fontFamily: "'JetBrains Mono', monospace",
                  padding: '2px 8px',
                  border: `1px solid ${trendData.stats.trend >= 0 ? 'var(--success)' : 'var(--error)'}40`,
                  color: trendData.stats.trend >= 0 ? 'var(--success)' : 'var(--error)',
                  background: `${trendData.stats.trend >= 0 ? 'var(--success)' : 'var(--error)'}14`,
                }}
              >
                {trendData.stats.trend >= 0 ? '↑' : '↓'} {Math.abs(trendData.stats.trend)}%
              </div>
            )}
          </div>
          {trendsLoading ? (
            ...
            <>
                  ...
                  >
                    <defs>
                      ...
                      <linearGradient id="noiseGrad" x1="0" y1="0" x2="0" y2="1">
                        <stop offset="5%" stopColor="var(--error)" stopOpacity={0.16} />
              ...
              >
                <span>
                  <span style={{ color: 'var(--success)' }}>—</span> Quality Score
                </span>
                <span>
                  <span style={{ color: 'var(--error)' }}>- -</span> Noise Level
                </span>
                <span style={{ marginLeft: 'auto' }}>
                  {trendData.stats.totalAnalyses} analyses · range {trendRange}
                </span>
              </div>
            </>
          )}
          ...
          <div className="card card-glow animate-slide-up" style={{ animationDelay: '0.66s' }}>
            ...
            >
              ...
              <span style={{ color: 'var(--success)', opacity: 0.7 }}>
                ■ target zone (high quality, low noise)
              </span>
            </div>
          </div>
        </ErrorBoundary>
      ...
      <div className="grid grid-2 gap-md" style={{ marginBottom: 'var(--spacing-lg)' }}>
        ...
        <div className="card card-glow animate-slide-up" style={{ animationDelay: '0.7s' }}>
          ...
          <div
            className="card-body"
            style={{ padding: 'var(--spacing-sm)', height: 260, overflowY: 'auto' }}
          >
        ...
        <div className="card card-glow animate-slide-up" style={{ animationDelay: '0.76s' }}>
          ...
          {insights.decisionTwinVotes.total > 0 ? (
            <>
              ...
              >
                Simulated votes from Fiscal Conservative · Aggressive Growth · Compliance Guard
                personas
              </div>
            </>
          ) : (
            ...
            {insights.avgBlindSpotGap > 0 && (
              <div
                style={{
                  ...
                  fontFamily: "'JetBrains Mono', monospace",
                  color:
                    insights.avgBlindSpotGap < 50
                      ? 'var(--error)'
                      : insights.avgBlindSpotGap < 80
                        ? 'var(--warning)'
                        : 'var(--success)',
                }}
              >
          ...
          <div className="card-body" style={{ padding: 'var(--spacing-md)' }}>
            {insights.avgBlindSpotGap > 0 && (
              <div style={{ marginBottom: '16px' }}>
                ...
                <div
                  style={{ height: '4px', background: 'var(--bg-secondary)', position: 'relative' }}
                >
                  <div
                    style={{
                      position: 'absolute',
                ...
                >
                  {insights.avgBlindSpotGap < 50
                    ...
                    : insights.avgBlindSpotGap < 80
                      ? 'Moderate Perspective Diversity'
      ...
      {graphTrends && graphTrends.weeklyData.length > 0 && (
        <>
          <SectionLabel index={9}>NETWORK INTELLIGENCE</SectionLabel>
      ...
      >
        ...
        <ShieldCheck
          size={10}
          style={{ display: 'inline', verticalAlign: 'middle', color: 'var(--success)' }}
        />{' '}
        System nominal
      </div>
