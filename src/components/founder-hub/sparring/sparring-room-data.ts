...
export const SCENARIO_TEMPLATES: ScenarioTemplate[] = [
  ...
  // ── Calibration-claim probe (Tetlock / Superforecasting flavour, locked 2026-06-05) ──
  // Distinct from the 4 wrapper scenarios above (which counter "the engine is
  // copyable" with embeddedness/process/record). This one drills the OPPOSITE
  // failure: the founder OVERCLAIMING Tetlock/Brier under VC pressure. The win
  // is the restraint — the do-not-quote discipline (Brier != DQI; the 23% is a
  // human-team result; extremizing could backfire) is the signal. Canonical in
  // icp.ts SUPERFORECASTING_DO_NOT_QUOTE + POSITIONING_CALIBRATION_LEG.
  {
    personaId: 'preseed_vc_associate',
    mode: 'objection_handler',
    generatorHint:
      "Riya just said: \"You're leaning on Tetlock now — Brier scores, the Good Judgment Project. Isn't this just Superforecasting-as-a-service with a memo on top? And that Brier number — is that your result, or are you borrowing Tetlock's credibility?\" The calibration-claim probe (a sharper, post-Superforecasting flavour of the wrapper challenge). You have 30 seconds, and the WIN is the restraint. Three beats: (1) DRAW THE HONEST LINE FIRST — \"Brier scores OUR calibration on the forecasts — the 90-day proxies and the 143-case predictions — not the DQI. The DQI grades the reasoning; Brier grades whether we're calibrated. I won't conflate them, because you'd catch it.\" (2) NAME the real structure — \"Tetlock isn't the product; it's the third leg. Kahneman catches the bias, Klein amplifies the expert read, Tetlock measures whether the call was right and sharpens the next one — forced 90-day proxies logged at the vote, Brier-scored at horizon, per-org recalibration. That measurement loop is the part a GPT-N can't copy by stealing prompts — it needs embedded outcome data. Active Open-Mindedness — capturing the user's prior before the algorithm reveals its read — already ships as the Intelligent Antagonist.\" (3) HOLD THE LINE THAT WINS HER — \"And here's what I'm NOT going to claim: the Good Judgment Project's '23% more accurate' is a human-team result; our jury is decorrelated model frames, not a superteam. I won't quote a platform Brier as proof of accuracy in a cold pitch. And extremizing the aggregate — Tetlock's famous trick — could actually make a decorrelated jury MORE overconfident, so it's deferred, not shipped.\" The discipline IS the pitch — she's hunting for the one founder who won't overclaim in a diligence room. Don't quote the 23%. Don't quote Brier 0.258. Don't call the noise jury a superteam. Don't say \"great question.\"",
  },
];
