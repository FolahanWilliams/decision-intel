 ...
 * canonical registry so cold-context buyer scripts cannot drift
 * silently when a new African / EM framework lands.
 */
...
export const KILLER_RESPONSES: KillerResponse[] = [
  ...
  },
  {
    ...
    responseCategory:
      'Category Contrast · engine vs accumulating asset (concede the wrapper, relocate the company)',
    exactPhrasing: `"You're right — the audit engine is a wrapper. Any competent team rebuilds it in a few weeks. So the question is not whether the engine is a moat — it isn't. The question is what else is in the bundle. Five ingredients: the wrapper, my founder narrative, time embedded at a specific firm, real calibrated outcome data from that firm, and network access through that firm's leadership. Only one of those — the wrapper — is replicable by a competitor with twenty million dollars and twelve months. The other four are not. A 35-year-old McKinsey alum cannot rebuild a 16-year-old solo founder rooted in Lagos with published research at 16 and a financial-literacy program. A competitor starting in 2027 cannot replay a 12-week in-person embed I am doing in summer 2026. They cannot fabricate the calibration data without an equivalent partner relationship, and they cannot warm-intro through a board I do not have access to. The moat is the COMBINATION. The wrapper is ingredient one — the wedge. The bundle is the company. The next twelve months are about locking the four unrepeatable ingredients, which means embedding deeply in one or two organisations and accumulating real calibrated decision-to-outcome data. If I cannot, the bundle stays incomplete and acqui-hire is the right outcome. If I can, four of five layers of competitive defence are in place. The wrapper question collapses the moment you stop measuring the engine and start counting what is around it."`,
    whyItWorks: `The 2026-05-21 sharpening — leads with the COMBINATION framing instead of the prior "engine vs accumulating asset" framing. Conceding the wrapper still disarms the challenge (denial would fail instantly), but the move that follows is no longer "the company is somewhere else" — it's "the company is FIVE ingredients of which the wrapper is one." That count anchors the entire frame: 4 of 5 are unrepeatable. The advisor then has to argue against the bundle, not against the engine — much harder, because the founder narrative + time embed + calibration data + network access are structurally outside their reach. Acqui-hire-vs-scale binary still gets dissolved into the 12-month experiment, but now the experiment has a specific moat-completion outcome (lock the 4 unrepeatable ingredients), not just "see if data accumulates." Stronger because it gives the advisor a concrete way to measure success that does not depend on revenue alone.`,
    followUpMove:
      'Name the linchpin concretely: "The single highest-leverage move toward locking 3 of 5 unrepeatable ingredients simultaneously is the partnership I am working toward with a Pan-African wealth management firm — a 12-week in-person embed at their London office. That activates ingredient three (time embedded), ingredient four (real calibrated data from auditing 30-50 of their historical closed decisions), and ingredient five (network access to the founder + her board). Combined with ingredient two (founder narrative) and ingredient one (the shipped wrapper), four of five bundle pieces lock in by month 12. The wrapper question collapses when you count what is around it." (See COMBINATION_INGREDIENTS in defensibility-vectors.ts + the SankoreBrief partnership-shape panel for the full bundle ledger.)',
  },
];
