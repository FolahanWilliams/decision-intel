...
- [ ] **Founder-side: confirm `npx prisma migrate deploy` has run against production (if not, run it once — it applies every pending additive migration cumulatively: founder-os daily goals + period goals/reflections [2026-06-01], analysis-quota reservation [2026-06-06], 3× SAT prep [2026-06-07], 66-Day Protocol reality-checkin [2026-06-14]).** All additive, zero-risk; pre-migration code fails soft. Consolidated here 2026-06-11 when the per-ship reminders were trimmed from Recently Completed.
