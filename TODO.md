...
- [ ] **FOUNDER — for the retro batch:** set `PIPELINE_BLIND_MODE=on` in Vercel env (Production) → redeploy → run the historical-filing retros → REMOVE the var (or set `off`) before any live/current-deal audits. The honest pitch line: "live retrieval was disabled; every finding cites the filing's own language" — never "the model couldn't have known."
