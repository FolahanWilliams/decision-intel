...
- **Inline live co-edit (2026-04-23):** On [InlineAnalysisResultCard](src/components/analysis/InlineAnalysisResultCard.tsx), a collapsible "Rewrite a flagged passage" panel sits between the biases list and the Featured Counterfactual. Expands to a textarea + Re-audit button; POSTs to [/api/passages/re-audit](src/app/api/passages/re-audit/route.ts) which runs a single focused Gemini call on the passage and returns `{ biases[], estimatedDqi, delta, disclaimer }`. Deliberately NOT the 12-node pipeline — passage-level estimate only, clearly labelled, ~£0.02-0.05 per call vs ~£0.40 for a full audit. Rate-limited 20/hr/user. When a user wants the canonical DQI, they upload the revised memo via the main flow.
