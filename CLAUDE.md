...
- **Landing hero shape (locked 2026-04-23):** Eyebrow → H1 in `.marketing-display` Instrument Serif (40-72px clamp, italic on the green "boardroom strategic decision" fragment) → 20-word subhead → HeroCounterfactualTease → primary "One free audit. Paste your memo." + secondary "How it works" → `HeroCredibilityStrip` (4 signals: R²F, AI Verify alignment, SOC 2 Type II infrastructure, Wiz advisor) → `SampleAuditCard` (static DQI 68 / 4 biases / what-if +18pp / DPR reference number). `HeroCredibilityStrip` + `SampleAuditCard` are both co-located helpers at the bottom of [page.tsx](<src/app/(marketing)/page.tsx>). **Rule: the hero must close with trust signal + visual weight, not with a dangling what-if chip.**
