...
- Per-audit API cost on Gemini paid tier 1: **~£0.30–0.50 (~$0.40–0.65)**, not the $0.03–0.07 previously claimed. Each audit fires ~17 LLM calls across 12 nodes. Cost-tier routing (Apr 2026) moved gdprAnonymizer/structurer/intelligenceGatherer to gemini-3.1-flash-lite for 15–25% savings. Pricing tiers (2026-04-15): Free (4/mo) → **Individual $249/mo** (15 audits, renamed from "Professional" and repriced from $149 to re-anchor away from productivity SaaS) → Strategy $2,499/mo (unlimited + team) → Enterprise custom. At $249 with ~£6/mo raw cost on 15 audits, margin is ~97% — but re-validate before any investor deck use.
- **Hidden cost sources:** `WeeklyBrief` + `ContentOpportunities` in `ContentStudioTab` are user-triggered (button click, not mount) — **verified Apr 2026**, no longer an auto-fire concern. `daily-linkedin` cron was burning budget pre-error due to email-delivery failure — disabled in dispatch Apr 2026. 6 files hardcode `gemini-2.0-flash` as fallback ([gemini.ts:50](src/lib/ai/providers/gemini.ts#L50) + content routes + outcome-inference) — this is why Gemini 2 Flash shows in usage charts even though the LangGraph pipeline uses gemini-3-flash-preview. Intentional for now; content-gen routes don't need Flash 3 preview.
...
- **Decision Log unification:** `/dashboard/decision-log` merges Journal (from `/api/journal`) + Cognitive Audits (via `useHumanDecisions` → `/api/human-decisions`) into one page with All / Journal / Audits chip filters and a pending / completed / dismissed status segmented control. `/dashboard/journal` is a redirect. Sub-routes `/dashboard/cognitive-audits/submit`, `/[id]`, `/effectiveness` untouched. Sidebar item "Decision Log" (pathname matches `/decision-log`, `/journal`, `/cognitive-audits`).
...
- **Founder School curriculum:** 8 tracks × variable lessons (currently 58 total) in [src/lib/data/founder-school/lessons.ts](src/lib/data/founder-school/lessons.ts). Standard lesson shape: `insight` / `whyItMatters` / `action` / `reflection`. The **Platform Foundations** track carries three extra optional fields — `sources` (primary-source references), `csoPitch`, `vcPitch` — rendered by [FounderSchoolTab](src/components/founder-hub/FounderSchoolTab.tsx) only when present. That track is the founder's methodology-consolidation curriculum (Kahneman-Tversky, Noise, DQI components, pre-mortems, Tetlock outcome loop, 20×20 matrix, 12-node pipeline, regulatory frameworks, 135-case library). Header count is dynamic via `TOTAL_LESSONS`, so adding lessons can't drift the display.
...
- **Landing-page credibility trio:** [src/components/marketing/CredibilityTrio.tsx](src/components/marketing/CredibilityTrio.tsx) — 3-card section between "How It Works" and "Features" on the landing page, linking to `/proof`, `/bias-genome`, and `/privacy`. Each card has a self-contained SVG thumbnail (proof = memo with 3 red flags; genome = mini leaderboard; privacy = lifecycle chips → vault lock) so the surface reads visually even before any page loads. This was the Bias Genome's first-class landing slot (it was previously only a footer link).
