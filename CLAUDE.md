...
- **Public Enterprise quote builder (2026-04-25):** [/pricing/quote](<src/app/(marketing)/pricing/quote/page.tsx>) is a public mirror of the admin-only enterprise quote builder. Mounts the same [EnterpriseQuoteBuilderClient](src/components/billing/EnterpriseQuoteBuilderClient.tsx) (now parameterised with `apiEndpoint`, `backHref`, `backLabel` props) and POSTs to [/api/billing/enterprise-quote-public](src/app/api/billing/enterprise-quote-public/route.ts) — IP rate-limited 5/IP/24h, no auth, requires customerName + plausible contactEmail before generating. Backwards-compat: the admin route at [/dashboard/settings/billing/enterprise-quote](<src/app/(platform)/dashboard/settings/billing/enterprise-quote/page.tsx>) and admin-only POST [/api/billing/enterprise-quote](src/app/api/billing/enterprise-quote/route.ts) stay untouched. The Pricing page Enterprise card surfaces the public route via a secondary link under the "Contact sales" CTA. `noindex` on the public page so it doesn't compete for search relevance with the rest of the marketing surface.
...
- **`/demo` paste-first flow (2026-04-25 P0 restore + P1 EM-lead update):** the `/demo` page is a server component at [src/app/(marketing)/demo/page.tsx](<src/app/(marketing)/demo/page.tsx>) that wraps the client [DemoPasteFlow](src/components/marketing/demo/DemoPasteFlow.tsx) inside a `Suspense` boundary (the client uses `useSearchParams`). Visitor pastes a strategic memo (≥50/≤4,000 words), clicks **Run free audit**, and the REAL 12-node pipeline runs via [/api/demo/run](src/app/api/demo/run/route.ts) — anonymous, encrypted at rest, rate-limited to 1 audit / IP / 24h with a 50/day global ceiling. On success the existing [PasteAuditResults](src/components/marketing/demo/PasteAuditResults.tsx) component renders the wow-sequence inline (DQI hero + biases + counterfactual + boardroom + post-result CTA). Four featured sample chips load from [SAMPLE_BUNDLES](src/lib/data/sample-bundles.ts), now leading with the African PE/VC samples — `pe-lagos-consumer-rollup` (Pan-African consumer-staples roll-up, EM overlay), `pe-kenya-fintech-growth` (Series-B fintech with multi-jurisdiction regulatory risk), `cso-brazil-market-entry` (LATAM developed-EM bridge), `ma-project-atlas-ic-memo` (classic IC memo). `?sample=<slug>` deep-links from `/case-studies/sample/[slug]` pre-populate the textarea. A live-pipeline phase progress strip animates while the audit runs (4-9 second phase durations to match real pipeline cadence). Below the paste shell, a strip of pre-loaded famous-decision audits links to `/demo/[slug]` for static deep views (Kodak / WeWork / Nokia / etc., resolved server-side via `ALL_CASES.filter(c => c.preDecisionEvidence != null && c.dqiEstimate != null)`). The historical regex `scanForBiases` path stays deleted — if a fallback is needed, use the real endpoint with a disabled flag rather than reintroducing a regex stub. **Critical:** every hero CTA across the marketing site (`MarketingNav`, landing-page hero + final CTA, `simulate-ceo` page) targets `/demo` flat — between the previous deletion and this restore the site shipped a 404 on its primary CTA for an unknown window. Do not delete this route again without first changing every `href="/demo"` to a slug-specific URL or replacement entry point.
...
- **Marketing copy discipline — no named-prospect leaks (locked 2026-04-25):** This is the single most important discipline rule on marketing surfaces. **Never** ship a public route, page title, JSON-LD entry, metadata description, or visible HTML copy that names a specific prospect or design-partner ("Sankore", "Wiz", any other firm we have not closed publicly), or any "{firm}-class buyers" coding pattern. Two real catches in the 2026-04-25 audit: `security/page.tsx:464` literally rendered "Sankore-class buyers" in the hero subhead, and the persona-audit's recommended "promote the Sankore brief to a public `/sankore` page" was queued before the founder caught it. The right delivery shape for a prospect-specific brief is a private channel (PDF email, token-gated link with `noindex`, in-call screen-share) or a generic page that doesn't name the prospect (e.g. `/for-funds`). The brief in [src/components/founder-hub/sankore/](src/components/founder-hub/sankore/) stays inside the founder hub. When a future audit recommends building a public page named after any prospect, evaluate against this rule first — the persona is recommending a need, not a delivery shape.
